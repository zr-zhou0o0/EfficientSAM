...
# load an image
sample_image_np = np.array(Image.open("data/00003.png"))
sample_image_tensor = transforms.ToTensor()(sample_image_np)
sample_image_np_2 = np.array(Image.open("data/00007.png"))
sample_image_tensor_2 = transforms.ToTensor()(sample_image_np_2)
images_tensor = torch.stack([sample_image_tensor, sample_image_tensor_2])
...
for model_name, model in models.items():
    ...
    print('predicted logits shape:', predicted_logits.shape) # (1,2,3,1200,1600)
    print('predicted iou shape:', predicted_iou.shape) # (1,2,3)

    sorted_ids = torch.argsort(predicted_iou, dim=-1, descending=True)
    predicted_iou = torch.take_along_dim(predicted_iou, sorted_ids, dim=2)
    predicted_logits = torch.take_along_dim(predicted_logits, sorted_ids[..., None, None], dim=2)
    print('predicted iou shape after tk:', predicted_iou.shape)
    print('predicted logits shape after tk:', predicted_logits.shape)
    ...
    Image.fromarray(masked_image_np).save(f"figs/examples/test0_{model_name}_mask.png")
    
    mask2 = torch.ge(predicted_logits[0, 1, 0, :, :], 0).cpu().detach().numpy()
    masked_image_np_2 = sample_image_np_2.copy().astype(np.uint8) * mask2[:,:,None]
    Image.fromarray(masked_image_np_2).save(f"figs/examples/test1_{model_name}_mask2.png")